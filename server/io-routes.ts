...
// No authentication required. CORS already handled globally.
// Returns io_employees + io_attendance in the consuming site's
// expected schema: { items: [{ id, title, category, description, url, updatedAt }] }
// ============================================================

const publicRouter = Router();

publicRouter.get("/data", async (req: Request, res: Response) => {
  try {
    const db = await getDb();
    if (!db) return res.status(500).json({ error: "Database not available" });

    // Determine the base URL for deep-links back to this site
    const origin = (req.headers.origin as string)
      || (req.headers.referer ? new URL(req.headers.referer as string).origin : '')
      || `${req.protocol}://${req.get('host')}`;
    const siteBase = origin || 'https://play-book.manus.space';

    // Optional query params for filtering
    const category = req.query.category as string | undefined;  // "employee" | "attendance"
    ...
    if (!category || category === 'employee') {
      ...
      for (const emp of empRows) {
        items.push({
          ...
          // Extended fields for richer consumption
          meta: {
            ohr_id: emp.ohr_id,
            ...
            employment_status: emp.employement_status,
            hire_date: emp.hire_date,
            work_off: emp.work_off,
          },
        });
      }
    }
      ...
      for (const att of attRows) {
        items.push({
          ...
          meta: {
            ...
            tag: att.tag,
            upl_reason: att.upl_reason,
            remarks: att.remarks,
            ot_hours: att.ot_hours,
            role: att.role,
            planning_group: att.planning_group,
            snap_full_name: att.snap_full_name,
            snap_supervisor: att.snap_supervisor,
    ...
    res.json({
      ...
      limit,
      offset,
      categories: ['employee', 'attendance'],
    });
  } catch (err: any) {
...
export function registerIORoutes(app: import("express").Express) {
  ...
  console.log("[IO API] Routes registered under /api/io/* and /api/public/*");
}
