...
function normalizeRecord(att) {
  ...
  return {
    ...
    agent: (emp.full_name || '').trim(),
    flm: (emp.supervisor_name || '').trim(),
    role: (emp.actual_role || '').trim(),
    actualPlanningGroup: (emp.planning_group || '').trim(),
    shiftTime: (emp.shift_time || '').trim(),
    status: (emp.employement_status || '').trim(),
