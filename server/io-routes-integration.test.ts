...
describe("IO Routes — Employees", () => {
  ...
  it("GET /employees returns employee list from DB", async () => {
    const testEmployees = [
      { ohr_id: "740001001", full_name: "Test Agent", planning_group: "S-ABF", shift_time: "GY Shift" },
      { ohr_id: "740001002", full_name: "Another Agent", planning_group: "CS-ABF", shift_time: "Mid-Shift" },
    ];
    mockResolvedRows.push(...testEmployees);

    const mod = await import("./io-routes.js");
    // The module exports registerIORoutes which mounts the router on an app
    // We need to get the router's route handlers directly
...
describe("IO Routes — Separate Route Modules (source verification)", () => {
  it("tardiness routes module is imported in _core/index.ts", async () => {
    const fs = await import("fs");
    const source = fs.readFileSync("/home/ubuntu/playbook/server/_core/index.ts", "utf-8");
    expect(source).toContain("registerTardinessRoutes");
  });

  it("role-change routes module is imported in _core/index.ts", async () => {
    const fs = await import("fs");
    const source = fs.readFileSync("/home/ubuntu/playbook/server/_core/index.ts", "utf-8");
    expect(source).toContain("registerRoleChangeRoutes");
  });

  it("managers-nook routes module is imported in _core/index.ts", async () => {
    const fs = await import("fs");
    const source = fs.readFileSync("/home/ubuntu/playbook/server/_core/index.ts", "utf-8");
    expect(source).toContain("registerManagersNookRoutes");
  });

  it("group-task routes module is imported in _core/index.ts", async () => {
    const fs = await import("fs");
    const source = fs.readFileSync("/home/ubuntu/playbook/server/_core/index.ts", "utf-8");
    expect(source).toContain("registerGroupTaskRoutes");
  });

  it("shift-extension routes module is imported in _core/index.ts", async () => {
    const fs = await import("fs");
    const source = fs.readFileSync("/home/ubuntu/playbook/server/_core/index.ts", "utf-8");
    expect(source).toContain("registerShiftExtensionRoutes");
  });

  it("performance routes module is imported in _core/index.ts", async () => {
    const fs = await import("fs");
    const source = fs.readFileSync("/home/ubuntu/playbook/server/_core/index.ts", "utf-8");
    expect(source).toContain("performanceRouter");
  });

  it("requireAuth middleware is mounted before IO routes", async () => {
    const fs = await import("fs");
    const source = fs.readFileSync("/home/ubuntu/playbook/server/_core/index.ts", "utf-8");
    // Find the actual app.use line for requireAuth (line 80) and registerIORoutes (line 81)
    const lines = source.split('\n');
    const authLine = lines.findIndex(l => l.includes("requireAuth") && l.includes("api/io"));
    const routeLine = lines.findIndex(l => l.includes("registerIORoutes(app)"));
