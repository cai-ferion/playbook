...
export default function CompassLayout({
        ...
        <div className="flex flex-col items-center gap-8 p-8 max-w-md w-full">
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-sm text-muted-foreground text-center max-w-sm">
              Coaching & Corrective Action Management. Sign in to continue.
            </p>
          </div>
          <Button
            onClick={() => {
  ...
  return (
    ...
    >
      <CompassLayoutContent setSidebarWidth={setSidebarWidth}>
        {children}
      </CompassLayoutContent>
    </SidebarProvider>
  );
}

function CompassLayoutContent({
  children,
  setSidebarWidth,
}: {
  children: React.ReactNode;
  ...
  // Resolve employee profile for role display
  const empQuery = trpc.compass.currentEmployee.useQuery(undefined, {
    retry: false,
  ...
  const activeMenuItem =
    ...
    menuItems.find(
      (item) => item.path !== "/compass" && location.startsWith(item.path)
    );
                ...
                <button className="flex items-center gap-3 rounded-lg px-1 py-1 hover:bg-accent/50 transition-colors w-full text-left group-data-[collapsible=icon]:justify-center focus:outline-none focus-visible:ring-2 focus-visible:ring-ring">
                  ...
                  <div className="flex-1 min-w-0 group-data-[collapsible=icon]:hidden">
                    <p className="text-sm font-medium truncate leading-none">
                      {empQuery.data?.full_name ?? user?.name ?? "-"}
                    </p>
                    <p className="text-xs text-muted-foreground truncate mt-1">
                      {empQuery.data?.actual_role ?? "Loading..."}
                    </p>
                  </div>
                </button>
              </DropdownMenuTrigger>
